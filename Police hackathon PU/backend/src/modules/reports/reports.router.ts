...
router.get('/investigations', authenticate, (req: Request, res: Response) => {
  ...
  return ResponseUtil.success(res, investigationsReport);
});

// GET /api/v1/reports/financial
router.get('/financial', authenticate, (req: Request, res: Response) => {
  ...
  return ResponseUtil.success(res, financialReport);
});

// [M4] Listings use a seeded pseudo-random generator for deterministic output.
// A simple mulberry32 PRNG seeded with a fixed value.
...
router.get('/listings', authenticate, (req: Request, res: Response) => {
  ...
  const listings = Array.from({ length: 25 }, (_, i) => ({
    ...
    priceUSD: Math.floor(rng() * 8000 + 500),
    priceBTC: +(rng() * 0.2 + 0.01).toFixed(4),
    ...
    scraped: `${Math.floor(rng() * 12 + 1)}h ago`,
  }));
  return ResponseUtil.success(res, listings);
});

// GET /api/v1/reports/alerts
router.get('/alerts', authenticate, (req: Request, res: Response) => {
  const store = storeManager.getStore();
  return ResponseUtil.success(res, store.alerts);
});

// PATCH /api/v1/reports/alerts/:id/acknowledge
router.patch('/alerts/:id/acknowledge', authenticate, (req: Request, res: Response) => {
  const store = storeManager.getStore();
  const { id } = req.params;
  const alert = store.alerts.find((a) => a.id === id);

  // [C5] Sanitized error — don't reflect user-provided ID.
  if (!alert) {
    return ResponseUtil.error(res, 'ALERT_NOT_FOUND', 'Alert not found.', 404);
  }

  alert.acknowledged = true;
  return ResponseUtil.success(res, alert);
});
