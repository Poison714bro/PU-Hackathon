...
export async function POST(request: Request) {
  ...
  try {
    ...
    const pythonPath = fs.existsSync(winPython) ? winPython : fs.existsSync(posixPython) ? posixPython : 'python';
    ...
    const dossier = {
      ...
      financialProfile: {
        totalVolumeUSD: ledgerData.total_volume_usd || 54000,
        peakOperationPeriod: ledgerData.peak_operation || "May 2025",
        genesisDate: ledgerData.genesis_date || "2023-01-15",
        coinJoinRounds: ledgerData.coinjoin_rounds_30d || 14
      }
    };
